    ...
    <tr className="hover:bg-slate-50/50 transition-colors h-[80px]">
      ...
      <td className="px-8 py-5 text-right">
        <div className="flex justify-end gap-2">
          {isEditing ? (
            <>
              <button onClick={() => onSave(f.id, editStock, editPrice)} className="p-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all shadow-md active:scale-95"><Check size={16} /></button>
            ...
            <button onClick={() => onEdit(f)} className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-600 hover:text-white rounded-lg transition-all shadow-sm active:scale-95"><Edit3 size={16} /></button>
          )}
        </div>
      </td>
    </tr>
...
export const InventoryList: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <ViewportAware placeholderHeight="500px" className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
