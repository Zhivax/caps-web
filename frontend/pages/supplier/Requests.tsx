...
export const Requests: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-black text-slate-800 uppercase tracking-widest flex items-center gap-3">
            <OrderIcon className="text-indigo-600" size={24} />
            Incoming Orders
          </h3>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 ml-9">Process and verify incoming UMKM requests</p>
        </div>
        <span className="px-5 py-2.5 bg-indigo-100 text-indigo-700 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm">
      ...
      {activeRequests.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6">
          {activeRequests.map((req) => {
            ...
            return (
              <div key={req.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-8 hover:border-indigo-100 transition-all group animate-in fade-in">
                <div className="flex-1 space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-6 px-6 py-4 bg-slate-50/50 rounded-2xl border border-slate-100/50">
                    <div className="flex items-center space-x-3">
                      <div className="p-2 bg-indigo-100 text-indigo-600 rounded-xl"><Package size={20} /></div>
                      <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">Material</p>
                  ...
                  {!canApprove && isVerificationNeeded && (
                    <div className="p-3 bg-rose-50 border border-rose-100 text-rose-600 text-[10px] font-black uppercase rounded-xl flex items-center gap-2">
                ...
                <div className="flex flex-col gap-3 min-w-[220px]">
                    ...
                    <div className="space-y-3">
                        {req.paymentProof ? (
                            <button 
                                ...
                                className="w-full flex items-center justify-center gap-2 p-3 bg-white border border-indigo-200 text-indigo-600 rounded-xl text-[10px] font-black uppercase hover:bg-indigo-50 transition-all"
                            >
                                <ImageIcon size={16} /> View Payment Proof
                            </button>
                        ) : (
                            <div className="p-3 bg-rose-50 border border-rose-100 text-rose-600 text-[9px] font-black uppercase rounded-xl flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            <button 
                                ...
                                className="flex items-center justify-center space-x-2 py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 shadow-xl shadow-emerald-50 transition-all active:scale-95 disabled:opacity-50"
                            >
                                <Check size={18} /> Approve Payment
                            </button>
                            <button 
                                onClick={() => updateRequestStatus(req.id, RequestStatus.REJECTED)}
                                className="flex items-center justify-center space-x-2 py-3 bg-white border border-rose-200 text-rose-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-50 transition-all"
                            >
                  ...
                  {isApproved && (
                    <button 
                      ...
                      className="flex items-center justify-center space-x-2 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95"
                    >
                  ...
                  {isShipped && (
                    <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 text-center">
                        ...
                        <p className="text-[9px] font-bold text-emerald-400 mt-1 uppercase">UMKM will confirm receipt</p>
                    </div>
                  )}
      ...
      {/* Image Preview Modal */}
      {selectedProof && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-slate-900/90 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="relative max-w-2xl w-full bg-white rounded-[2rem] p-4 flex flex-col">
                <div className="flex justify-between items-center mb-4 px-4 pt-2">
                    <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <ShieldCheck size={16} className="text-emerald-500" /> Payment Confirmation Detail
                    </h5>
                    <button onClick={() => setSelectedProof(null)} className="p-2 bg-slate-100 text-slate-500 rounded-full hover:bg-rose-500 hover:text-white transition-all">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="mt-6 flex justify-center pb-2">
                    <button onClick={() => setSelectedProof(null)} className="px-10 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-100 transition-all hover:scale-105 active:scale-95">
                        Back to Requests
                    </button>
                </div>
            </div>
