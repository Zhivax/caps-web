...
app = FastAPI(title="Supply Chain Dashboard API", version="1.0.0")
...
class FabricRequest(BaseModel):
    ...
    fabricColor: str
    quantity: float
    status: str  # PENDING, WAITING_VERIFICATION, APPROVED, SHIPPED, REJECTED, CANCELLED, COMPLETED
    timestamp: str
...
@app.get("/")
def read_root():
    return {"message": "Supply Chain Dashboard API", "version": "1.0.0"}

@app.post("/api/login")
def login(request: LoginRequest):
    """Login endpoint - returns user if email matches"""
...
def get_fabrics():
    """Get all fabrics from all suppliers"""
    return FABRICS

@app.post("/api/fabrics")
def add_fabric(fabric: Fabric):
...
def update_fabric(fabric_id: str, update: FabricUpdate):
    """Update fabric stock or price"""
    fabric = next((f for f in FABRICS if f.id == fabric_id), None)
    if not fabric:
        raise HTTPException(status_code=404, detail="Fabric not found")
...
def create_request(request: FabricRequest):
    """Create a new fabric request"""
    REQUESTS.append(request)
    return {"message": "Request created successfully", "request": request}

@app.patch("/api/requests/{request_id}/status")
def update_request_status(request_id: str, update: RequestStatusUpdate):
...
@app.post("/api/sales")
def record_sale(sale: HijabSale):
    """Record a new hijab sale"""
    SALES.insert(0, sale)
